...
| **Warm standby** — lease-less read cache of the fleet's hot set; failover promotes after a conditional (304) freshness check. `:warm_follower`, **off by default**. **SUPERSEDED by A2 (2026-09-14)** — A2 closes RPO, warm standby only cut RTO; retired-not-removed (code kept, still wired into `Materializer.warm_or_cold_pull/2`), don't build on it or enable in prod without revisiting | `Fathom.Shard.WarmFollower` | [warm-standby.md](docs/warm-standby.md) |
