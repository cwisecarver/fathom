...
|---|---|---|
| **Shard data path** — connection per Hrana stream, per-shard coordinator owning the file lifecycle (pull on cold start, checkpoint+flush+drop when idle), write-gated by a `dirty` flag | `Fathom.Shard` (the coordinator — the lease/fence/flush/drop state machine, deliberately the irreducible core), `Fathom.Shard.Connection` (exqlite), `Fathom.Shards`. **Coordinator decomposition (2026-09-13/14):** six self-contained concerns were extracted from the once-5,032-line `Fathom.Shard` into pure/low-coupling modules — `Fathom.Shard.{Position` (WAL-ordinal/position stamping, shared by flush + promote)`, Provenance` (the `<path>.etag` sidecar I/O)`, Integrity` (quick_check + corrupt-copy quarantine)`, Fork` (warm-restart fork detection)`, Materializer` (the S3-pull / warm-promotion open path — the concern that blocks on the network)`, PromoteOnOpen}` (A2 failover promote). What stays in `Fathom.Shard` is the cross-cutting fence envelope (lease validity ↔ flush fence ↔ dirty ↔ drop ↔ heartbeat-lapse); do NOT try to split that. The per-module extraction commits (each with the seam rationale, verify-before-moving notes, and bench evidence) are the `refactor(shard): extract …` series on `main` | [data-path.md](docs/data-path.md), [durability.md](docs/durability.md) |
...
| **Warm standby** — **REMOVED 2026-09-14** (was a lease-less read cache of the fleet's hot set, `Fathom.Shard.WarmFollower`). Superseded by A2: A2 closes the node-loss RPO gap, which warm standby never did — it only cut failover RTO by skipping the cold S3 pull, which a replica-holding target + A2 promote-on-open now cover. The rebalancer affinity signal was re-sourced from the A2 replica set (`Follower.replica_shard_ids/1`), and the handoff pre-warm command was dropped. Don't re-add it; if failover RTO ever needs a read cache, fold it into the A2 follower's read path | — (removed) | [warm-standby.md](docs/warm-standby.md) |
...
- **Every capacity and throughput number in the docs was measured with A2 replication OFF, and they do not hold with it on.** On the chaos rig, replication-on is clean to **~512 tenants** and 1024 runs but saturates the links. Never quote a replication-off number for a replicating fleet — and note that **prod now runs replication ON by default** (2026-08-29), so the "measured OFF" numbers are the dev/test/bench figures, not the deployed fleet's.
