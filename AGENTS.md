...
| **Live WAL replication (A2)** — quorum-replicated WAL frames over A2's own socket protocol + peer-recovery on failover. **On by default in prod** (ship + receive both; off in dev/test), so a prod node without `REPLICATION_FOLLOWERS`+`REPLICATION_BIND_IP` **fails to boot**. Ship and receive are **separate** gates; the ordinal + frame-auth **wire** flags stay staged env flips | `Fathom.Shard.Replication.*` | [a2-quorum-replication.md](docs/a2-quorum-replication.md) |
