...
- **Phase 2 remainder**: shard locality/affinity (C) — its **affinity-aware placement** piece is now **built** (folded into B1): a per-node **warm-location signal** (`Fathom.Rebalancer.WarmLocations` + `shard_warm_locations`) lets `Rebalancer.Policy.best_target` prefer a handoff target that already holds the shard warm, within a load band of the least-loaded so balance still improves (`:rebalance_locality_band`, default 0.5). What remains of C is C1 (rendezvous/bounded-load hashing — marginal per `docs/phase2-scoping.md` §C) and C2 (multi-region region-affinity — a separate initiative). **Live WAL streaming (A2) is NO LONGER deferred — it WORKS on the `a2-quorum-replication` branch and is simply not merged to `main`** (proven end to end on the chaos rig 2026-08-11: `smoke` passes with replication on). Don't cite it as a deferred-by-decision gap. Its blocker ("exqlite 0.37.0 exposes no WAL-frame API, so there is no seam to ship a frame from") was disproved 2026-08-09 by the same move as the Django UDFs: a **loadable extension** receives a live `sqlite3*`, and `sqlite3_wal_hook` is in the extension pointer table, so exqlite's surface was never the boundary. Watch the trap if you touch it — `sqlite3_wal_hook` and `wal_autocheckpoint` are the **same slot**, so registering a hook that merely observes silently disables checkpointing on every tenant connection and grows the WAL without bound. It needed **no BEAM cluster** (its own socket protocol; S3 stays the only cross-node *coordination*) and costs **+225 µs per write (4.04×)** on, noise off; membership is roster-derived (`REPLICATION_MEMBERSHIP=static|roster`) behind a swap that REFUSES any set below `quorum+1`, because `n` dropping under `q` raises inside a tenant's commit. Two more traps the rig found and unit tests could not: the follower LISTENER was never started outside tests (hence the separate `REPLICATION_LISTEN` gate, which must be on fleet-wide before any node ships), and a recreated WAL read as a rewind — `ckpt_seq` restarts at 0 when SQLite deletes the `-wal` on last-connection-close, so `salt1` must cross the wire or primary and follower deadlock forever. Every replication test held ONE connection, which is exactly why neither was visible; a stream boundary is the shape to test across. The warm standby (A1) and **dynamic rebalancing (B1)** are **built** — see the warm-standby and rebalancer bullets above. A `fathom_native` Rust **NIF** — still absent, and no Rustler dep. But **`native/` is no longer empty**: `native/fathom_udf` (the Django UDF + WAL-hook extension) is a **loadable SQLite extension**, not a NIF, loaded via `enable_load_extension/2` rather than linked into the BEAM. Don't read "no `native/`" here as licence to add Rustler — twice now the thing that unblocked a "we need a NIF" problem was the extension that already exists.
