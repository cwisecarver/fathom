...
3. `REPLICATION_LINEAGE_WIRE=true` **everywhere**, once every node is running code that understands
   the frame. Same ordering rule as steps 1-2 and the same failure if reversed — a peer one deploy
   behind answers `:malformed` and stops being seeded. Do it before step 4: without it, promotion
   compares two different counters and stays inert.
4. `REPLICATION_PROMOTE_ON_OPEN` next, after flushes have been stamping positions for a while.
5. `REPLICATION_RECOVER_FROM_PEERS` last. Step 4 only helps when the survivor happens to be holding
   a replica; this is what makes the RPO claim hold when it is not.
6. `REPLICATION_SIGN_FRAMES=true` **everywhere**, then `REPLICATION_HMAC_REQUIRED=true` everywhere.
