...
#### Fidelity — measured against Django, three ways

Compatibility here is a measurement, not a claim. Three differential suites, each generated by
running the reference implementation rather than by hand-writing what it is believed to do:

| Suite | Compares | Cases |
|---|---|---|
| `native/fathom_udf/tests/oracle.rs` | each `_sqlite_*` function's return value | 12,501 |
| `native/fathom_udf/tests/re_oracle.rs` | `regexp` against CPython's `re.search` | 3,444 |
| `test/fathom/django_sql_parity_test.exs` | **whole queries** against Django's backend | 42 |

The third is the strongest: it opens a real Python `sqlite3` connection, registers Django's UDFs on
it exactly as `django/db/backends/sqlite3/base.py` does, runs whole queries, and requires fathom to
return identical rows **and identical `typeof()`**. That covers what per-function comparison
structurally cannot — result types, which rows a `WHERE` clause returns, GROUP BY/ORDER BY, and
NULL columns.

Between them these found **eight real bugs** that hand-written expectations had missed, including
`django_format_dtdelta` returning text where Django returns a number, `date - timedelta` moving the
date a day when Python does not, and the `regexp` case below.

##### `regexp` is Python's `re`, not Rust's `regex`

Django's `__regex` / `__iregex` are `re.search`. Rust's `regex` crate differs from it in two ways,
and the second is the dangerous one:

- **Rejected patterns.** Lookahead `(?=)`, lookbehind `(?<=)` and backreferences `\1` are ordinary
  Python and are compile errors in `regex`. A queryset got an error where Django returned rows.
- **A silently opposite answer.** Python's `$` (without `re.MULTILINE`) matches at the end of the
  string *or just before a trailing newline*; Rust's matches only at the very end. So
  `re.search("^[a-z0-9-]+$", "a-slug-1\n")` is `True` in Django and was `false` here — same pattern,
  same subject, no error. Any text arriving from a form or a file carries that newline.

`native/fathom_udf/src/pyre.rs` closes both: `fancy-regex` for the backtracking constructs, plus a
small translation layer for the tokens Python spells differently (`$` → `(?=\n?\z)`, `\Z` → `\z`,
`(?P=name)` → `\k<name>`). Verified against 3,444 CPython-generated cases.

##### The two remaining differences, both deliberate

Both are cases where **Django raises and fathom answers**, so every query that works on Django works
here — the direction that matters. Documented in `native/fathom_udf/src/aggregates.rs` and asserted
explicitly in `django_sql_parity_test.exs` rather than tolerated:

- **`STDDEV_POP`/`VAR_POP` over a column containing NULL.** Django's aggregate step is
  `list.append`, so `statistics.pstdev` receives a `None` and raises `TypeError`. Reproducing that
  would mean a `StdDev` over any nullable column errors the tenant's query. We skip NULLs, as every
  other SQL engine does.
- **A degenerate group** (`VAR_SAMP` over one row). `statistics.variance` raises; we return NULL, as
  PostgreSQL does.

These are not reproduced faithfully on purpose. They are Python-library behaviour rather than
