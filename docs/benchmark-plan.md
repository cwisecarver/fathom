...
- **Hot paths to watch** (fathom's scaling story is *millions of small shards*, so cost is dominated by per-shard open and fan-out, not single-query throughput):
  ...
  - **Failover RTO — A2 promote-on-open vs cold (opt-in S3).** `failover_cold_s3_p50_us` (survivor cold-opens with a full S3 pull) vs `failover_promote_p50_us` (a pre-positioned A2 local replica is served by `Fathom.Shard.Replication.PromoteOnOpen` instead of a body GET), both at `:warm_size_kb`; `mix fathom.bench --only failover_rto`. **Replaces the removed warm-follower 304 arm** (`failover_warm_s3_p50_us`, retired 2026-09-14 with `WarmFollower`). **The honest delta:** promote is *not* purely local — it still snapshots pre-promotion state, reads the object head, and does a fenced PUT; the win is the object **body transfer avoided**, so it scales with shard size × bandwidth-delay and is **marginal for a tiny shard on a fat pipe**. Measured 2026-09-15 (dev build, MinIO+toxiproxy via `scripts/benchmark_s3_latency.sh`, *relative not prod-absolute*): tiny shard / 30 ms one-way / uncapped → **cold ~616 ms vs promote ~614 ms — a tie** (nothing to save on a fast link); 5 MB (`--warm-size-kb 5000`) / 30 ms / 4 MB·s⁻¹ cap → **cold ~1,523 ms vs promote ~1,163 ms (≈24%)**. **On the chaos rig, end-to-end failover RTO is lease-floor-bound, not S3-pull-bound** (`./chaos.sh failover`: **15,945 ms** ≈ TTL 10 s + steal margin 5 s), so promote's open-latency saving is a negligible fraction of real failover time. Promote's actual value there is **RPO, not RTO speed** — proven by `REPLICATION_ENABLED=true ./chaos.sh rpo`: an acked-but-unflushed write survives node loss **only** with survivor selection on (gate OFF → row lost; gate ON → survivor pulls the fresher replica from a peer and logs `PROMOTED a local replica over the stored object`). See `docs/a2-quorum-replication.md`. (Historical: the removed warm arm read cold ~162 / warm ~72 ms at 1 MB / 30 ms / 100 Mbps on 2026-07-01; not comparable to the promote arm.)
